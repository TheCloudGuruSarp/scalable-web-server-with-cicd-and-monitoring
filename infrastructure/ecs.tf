  ...
  container_definitions = jsonencode([
    {
      ...
      image     = "public.ecr.aws/bitnami/node:18" # Using a public placeholder that exists
      cpu       = 256
      memory    = 512
      essential = true
      portMappings = [
  ...
  container_definitions = jsonencode([
    {
      ...
      image     = "public.ecr.aws/bitnami/nginx:1.21" # Using a public placeholder that exists
      cpu       = 256
      memory    = 512
      essential = true
      portMappings = [
...
resource "aws_ecs_service" "frontend" {
  ...
  # Depends on the ALB listener to ensure it's created before the service.
...
resource "aws_ecs_service" "backend" {
  ...
  # Depends on the ALB listener rule to ensure it's created before the service.
